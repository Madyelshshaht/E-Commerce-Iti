...
const Categories = () => {
    const { categories, loading, error, RemoveCategory, AddCategory, EditCategory, } = useCategories();
    ...
    return (
        <div className="container mt-4">
            ...
            <div className="d-flex justify-content-between align-items-center mb-5 mt-3 overflow-hidden ">
                <Heading title="Categories" />
                {/* {isAdmin && ( <button className="btn btn-success" onClick={handleAdd}> {!toggle ? " Add Category " : "Close"} </button> )} */}
            </div>

            {/* {isAdmin && ( <FormCategory toggle={toggle} setToggle={setToggle} initialData={initialData} setInitialData={setInitialData} AddCategory={AddCategory} EditCategory={EditCategory} /> )} */}

            <Row>
                {/* Search */}
                <div>
                    <SearchInput data={categories} searchKey="title" placeholder="Search by Category Name" setSearchData={setSearchData} />
                </div>

                {displayedProducts.length > 0 ? (
                    displayedProducts.map((cat) => (
                        <Col lg={3} md={4} xs={6} className="d-flex justify-content-center mb-5 mt-3 " key={cat.categoryId} >
                            <Category id={cat.categoryId} title={cat.title} img={cat.imageData} prefix={cat.prefix} onEdit={() => handleEdit(cat)} onDelete={() => RemoveCategory(cat.categoryId)} />
                        </Col>
                    ))
                ) : (
                    <>
